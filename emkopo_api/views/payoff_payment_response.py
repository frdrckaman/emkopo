...
from emkopo_api.serializers import LoanPayoffPaymentResponseSerializer
...
class LoanPayoffPaymentResponseAPIView(APIView):
    """
    API View to send XML data for loan payoff payment response, fetch data from LoanTakeoverDetail,
    and insert it into the LoanPayoffPaymentResponse model.
    """

    @swagger_auto_schema(
        operation_description="Send XML for loan payoff payment response and insert into LoanPayoffPaymentResponse model.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={'LoanNumber': openapi.Schema(type=openapi.TYPE_STRING, description='LoanNumber ID')},
            required=['LoanNumber']
        ),
        responses={
    ...
    def post(self, request):
        return payoff_payment_response(request)

def payoff_payment_response(request):
    ...
    request_data = {
        ...
        "MessageType": "FULL_REPAYMENT_0FF_BALANCE_RESPONSE",
...
def generate_xml_for_response(request_data, fsp):
    """
    Generate XML data for loan payoff payment response.
    """
    serializer = LoanPayoffPaymentResponseSerializer(data=request_data)
    if serializer.is_valid():
        ...
        SubElement(header, "MessageType").text = "FULL_REPAYMENT_0FF_BALANCE_RESPONSE"
        ...
        SubElement(message_details, "PaymentReferenceNumber").text = "22211"
