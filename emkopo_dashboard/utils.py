...
class EmkopoTemplateDoesNotExist(Exception):
    pass


def get_bootstrap_version():
    try:
        bootstrap_version = settings.IDAP_BOOTSTRAP
...
def get_template_path_with_bootstrap(original_path, bootstrap_version=None):
    ...
    or raises IdapTemplateDoesNotExist.
    """
    app_name = None
    ...
    for app_config in django_apps.get_app_configs():
        if app_config.name in original_path:
            ...
            break
    if not app_name:
        raise EmkopoTemplateDoesNotExist(
        ...
        path_list.insert(1, f"bootstrap")
...
def select_idap_template(relative_path, default_app_label):
    """Returns a template object.
    """
    local_path = f"{settings.APP_NAME}/bootstrap/"
    default_path = f"{default_app_label}/bootstrap/"
